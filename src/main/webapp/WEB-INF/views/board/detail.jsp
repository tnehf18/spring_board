  ...
  <script type="text/javascript">
	$(function() {
		...
		$("#btn-modify").on("click", function() {
			...
			$("#detailForm").attr("action", "<c:url value='/board/modify' />");
			$("#detailForm").submit();
		});
		//삭제
		$("#btn-remove").on("click", function() {
			if (!confirm("정말 삭제하시겠습니까?")) {
				...
				$("#detailForm").attr("action", "<c:url value='/board/remove' />");
				$("#detailForm").submit();
			}
		});
	});
...
<body>
  ...
  <div class="container">
    ...
    <form id="detailForm" name="detailFormn" action="#" method="POST">
      ...
      <div class="form-group row">
        ...
        <div class="col-sm-10">
          <input type="text" class="form-control" id="title" name="title" placeholder="제목" value="${board.title }" readonly="readonly" />
        </div>
      </div>
      <div class="form-group row">
        <label for="content" class="col-sm-2 col-form-label">본문</label>
        <div class="col-sm-10">
          <textarea class="form-control" id="content" rows="10" readonly="readonly">${board.content }</textarea>
        </div>
      </div>
      <div class="form-group row">
        ...
        <div class="col-md-12">
          ...
          <c:if test="${sessionScope.member.id eq board.writer }">
            <div class="float-right">
              <input type="button" class="btn btn-success" id="btn-modify" value="수정" />
              <input type="button" class="btn btn-danger" id="btn-remove" value="삭제" />
            </div>
          </c:if>
        </div>
      </div>
